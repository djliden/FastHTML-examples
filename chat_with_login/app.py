...
def get(auth):
    ...
    top = Grid(H1(title), Div(theme_toggle, logout_button, style="text-align: right"))
    ...
    chat_input = Textarea(
        ...
        hx_swap="beforeend",
        hx_on="keydown[key=='Enter' && !shiftKey]:event.preventDefault()"

    )
    send_button = Button("Send", type="submit",
                         style="width:150px")
    ...
    input_form = Form(
        ...
        hx_target="#chat-history",
        hx_swap="beforeend",
        hx_trigger="submit, keydown[key=='Enter' && !shiftKey] from:#user_message",
    ...
    return Title(title), Container(content)


@app.post("/send-message")
def send_message(user_message: str):
    user_message_div = Div(f"User: {user_message}", cls="user-message")
    bot_response_div = Div("Bot: bot response", cls="bot-response")
    ...
    return user_message_div, bot_response_div, clear_input
